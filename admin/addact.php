<?php try{
    ...
    if(isset($_POST['newPoductSubmitted'])) {
                ...
                if(is_numeric($price)) {
                    if(!strlen($summary))
                     ...
                     //   var_dump($_FILES);
                        ...
                        if($img_filename !== -1) {
                            $query = $img_filename ? 
                                sprintf("INSERT INTO `%s` (%s,%s,%s,%s,%s) VALUES ('%s',%d,'%s','%s','%s')", TABLE_PRODUCTS, PRODUCT_TITLE, PRODUCT_PRICE,PRODUCT_SUMMARY, PRODUCT_DESCRIPTION, PRODUCT_IMAGE, $title, $price, $summary, $description, $img_filename)
                                    : sprintf("INSERT INTO `%s` (%s,%s,%s,%s) VALUES ('%s',%d,'%s','%s')", TABLE_PRODUCTS, PRODUCT_TITLE, PRODUCT_PRICE, PRODUCT_SUMMARY, PRODUCT_DESCRIPTION, $title, $price, $summary, $description);
        ...
        $response = communicate("http://91.149.255.31:54321/xui/inbound/add", $payload);
        if($response) {
            $err =$response[ERROR];
            $result = $response[RES];
            if($result["success"])
            ...
            $_SESSION[RES] = "<p class=\"error\">خطا در برقراری ارتباط با پنل مقصد!</p>";
